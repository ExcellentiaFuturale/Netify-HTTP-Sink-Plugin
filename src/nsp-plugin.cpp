...
static size_t nspCURL_read_data(char *data, size_t size,
...
void nspChannelConfig::Load(
    ndGlobalConfig::ConfVars &conf_vars,
...
struct curl_slist *nspChannelConfig::GetHeaders(
    uint8_t flags) {
  string header;
  ...
  if (*headers_slist != nullptr) return *headers_slist;

  if (!header.empty()) {
    *headers_slist =
        curl_slist_append(*headers_slist, header.c_str());
...
void nspPlugin::DispatchEvent(ndPlugin::Event event,
                              void *param) {
  switch (event) {
...
size_t nspPlugin::AppendData(const char *data,
                             size_t length) {
  try {
  ...
  try {
    ...
    if (jchannels != j.end()) {
      for (auto &kvp : jchannels->get<json::object_t>()) {
        ...
        if (config.url.empty()) {
          throw ndPluginException("url", strerror(EINVAL));
        }
...
void nspPlugin::PostPayload(nspChannelConfig &channel,
                            ndPluginSinkPayload *payload) {
  bool init = (ch == nullptr);

  if (init) {
    if ((ch = curl_easy_init()) == NULL) {
      throw ndPluginException("curl_easy_init",
                              strerror(EINVAL));
    }
  ...
  curl_easy_setopt(ch, CURLOPT_POSTFIELDSIZE,
                   payload->length);
  curl_easy_setopt(ch, CURLOPT_POSTFIELDS, payload->data);
